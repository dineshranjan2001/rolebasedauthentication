...
const sendSMStoMobileNumber = require("../utils/generateotpandsendtomobilenumber.utils");

const resetPasswordInMobileNumberService=async(phoneNumber)=>{
    ...
    if(updateStatus){
        const checkSendSMSStatus=await sendSMStoMobileNumber(otp,[`+91${getUserDetails.phoneNumber}`])
        return !!(checkSendSMSStatus);
    }else{
        return false;
    }
}

const resetPasswordInMobileService=async(newPassword,confirmPassword,otp)=>{
...
module.exports={resetPasswordInMobileNumberService,resetPasswordInMobileService}